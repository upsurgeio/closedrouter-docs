...
```bash
curl https://api.pineapple.com/v1/chat/completions \
...
Pineapple can force responses into a **strict JSON schema**, ensuring outputs are machine-readable.  \
...
```bash
curl https://api.pineapple.com/v1/images/generations \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "anthropic/claude-3-5-sonnet-20241022",
